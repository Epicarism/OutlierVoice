...
final class AlarmManager {
    ...
    func requestNotificationPermission() {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge, .criticalAlert]) { granted, error in
            if granted {
                print("[Alarm] ✅ Notification permission granted")
            } else {
                print("[Alarm] ❌ Notification permission denied: \(error?.localizedDescription ?? "unknown")")
            }
        }
    }
    ...
    private func scheduleNotification(for alarm: ClaudeAlarm) {
        ...
        content.title = "📞 Claude is calling!"
        ...
        if alarm.repeatDays.isEmpty {
            ...
            UNUserNotificationCenter.current().add(request) { error in
                if let error = error {
                    ...
                    print("[Alarm] ⏰ Scheduled one-time alarm for \(alarm.timeString)")
                }
            }
        } else {
            ...
            for day in alarm.repeatDays {
                ...
                UNUserNotificationCenter.current().add(request) { error in
                    if let error = error {
                        print("[Alarm] ❌ Failed to schedule day \(day): \(error)")
                    }
                }
            }
            print("[Alarm] ⏰ Scheduled repeating alarm for \(alarm.repeatDescription)")
        }
    }
    ...
    private func cancelNotification(for alarm: ClaudeAlarm) {
        ...
        print("[Alarm] 🚫 Cancelled notifications for: \(alarm.title)")
    }
    
    // MARK: - Alarm Triggering
    
    func triggerAlarm(_ alarm: ClaudeAlarm) {
        print("[Alarm] 🔔 TRIGGERING: \(alarm.title)")
        activeAlarm = alarm
        ...
        // 🔥 USE CALLKIT - Makes it ring like a real phone call!
    ...
    private func triggerAsPhoneCall(_ alarm: ClaudeAlarm) {
        ...
        callKit.onCallAnswered = { [weak self] in
            Task { @MainActor in
                print("[Alarm] 📞 User answered the call!")
                self?.startAlarmLoop()
        ...
        callKit.onCallEnded = { [weak self] in
            Task { @MainActor in
                print("[Alarm] 📴 User declined/ended the call")
                self?.dismissAlarm()
            }
        }
    ...
    func snoozeAlarm(minutes: Int = 5) {
        ...
        let content = UNMutableNotificationContent()
        content.title = "📞 Claude is calling again!"
    ...
    private func loadAlarms() {
        if let data = UserDefaults.standard.data(forKey: storageKey),
           let decoded = try? JSONDecoder().decode([ClaudeAlarm].self, from: data) {
            alarms = decoded
            print("[Alarm] 📂 Loaded \(alarms.count) alarms")
